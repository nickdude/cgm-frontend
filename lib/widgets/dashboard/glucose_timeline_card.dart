...
class GlucoseTimelineCard extends StatelessWidget {
  const GlucoseTimelineCard({
    super.key,
    this.currentTimeLabel = '10:25 am',
    this.highlightedValue = '176 mg/dL',
    this.timeTicks = const ['6 am', '9 am', '12 pm', '3 pm'],
  });
                  ...
                  child: Center(
                    child: Text(
                      ...
                      style: TextStyle(
                        ...
                        fontSize: 20,
            final safeTicks = timeTicks.isEmpty
                ? const ['6 am', '9 am', '12 pm', '3 pm']
                : timeTicks;
            const tickSlotWidth = 96.0;
            final chartWidth = math.max(constraints.maxWidth, safeTicks.length * tickSlotWidth);
                    ),
                  ),
                ),
                Positioned(
                  ...
                  child: Row(
                    children: [
                      final tickSlotWidth = 96.0;
                      final chartWidth = math.max(
                        constraints.maxWidth,
                        timeTicks.length * tickSlotWidth,
                      );
                    ...
                    mainAxisAlignment: MainAxisAlignment.spaceAround,
                        ...
                        .map(
                                ...
                                Positioned(
                                  ...
                                  right: 0,
                                  child: Center(
                                    child: _ValuePill(value: highlightedValue),
                                  ),
                                ),
                                Positioned(
                                  left: 0,
                                  ...
                                  child: Row(
                                    children: timeTicks
                                        .map(
                                          (tick) => SizedBox(
                                            width: chartWidth / timeTicks.length,
                                            child: Center(
                                              child: Text(
                                                tick,
                                                style: const TextStyle(
                    ...
                    Positioned(
                      ...
                      child: Row(
                        children: safeTicks
                            .map(
                              (tick) => SizedBox(
                                width: chartWidth / safeTicks.length,
                                child: Center(
                                  child: Text(
                                    tick,
                                    style: const TextStyle(
...
class _TimelinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final baseY = size.height * 0.70;
    _drawDashedLine(canvas, Offset(0, baseY), Offset(size.width, baseY), dash, dashLength: 6, gapLength: 5);
    _drawDashedLine(
      canvas,
      Offset(0, size.height - 8),
      Offset(size.width, size.height - 8),
  ...
  Color _lineColorForY(double y, double chartHeight) {
    ...
    if (t < 0.35) return Color.lerp(red, orange, t / 0.35)!;
    if (t < 0.65) return Color.lerp(orange, yellow, (t - 0.35) / 0.30)!;
    return Color.lerp(yellow, green, (t - 0.65) / 0.35)!;
  }
